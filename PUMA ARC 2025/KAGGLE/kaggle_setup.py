...
def _resolve_llm_model_path(working_root: Path, arc_solver_dir: Path) -> Optional[str]:
    """Return first available local LLM model path."""

    candidates = [
        working_root / "models_kaggle" / "phi-3-mini-4k-instruct",
        working_root / "models" / "phi-3-mini-4k-instruct",
        arc_solver_dir.parent / "models" / "phi-3-mini-4k-instruct",
    ]
...
def _build_llm_config(
    ...
    """Normalize LLM configuration payload for the solver."""

    if llm_options is None:
        llm_options = {}
    options = dict(llm_options)
    use_llm = options.pop('use_llm_reasoning', True)
    model_name = options.pop('model_name', None) or options.pop('model_path', None)
    if model_name is None:
        model_name = _resolve_llm_model_path(working_root, arc_solver_dir)

    if not use_llm:
        return {'use_llm_reasoning': False}
    if not model_name:
        return None
    ...
    config: Dict[str, Any] = {
        ...
        'llm_temperature': options.pop('temperature', 0.3),
        'llm_max_tokens': options.pop('max_tokens', 512),
        'use_4bit': options.pop('use_4bit', True),
    }

    if 'device' in options:
        config['llm_device'] = options['device']
    if 'cache_dir' in options:
        config['llm_cache_dir'] = options['cache_dir']
...
def get_solver(
    ...
    **kwargs
    ...
    Args:
        ...
        llm_options: Optional overrides for LLM usage (model, temperature, etc.)
        runtime_flags: Optional solver runtime directives (RFT, strategy, fallbacks)
    ...
    llm_config = _build_llm_config(llm_options, working_dir, arc_solver_dir)
    ...
    if llm_config is not None:
        solver_kwargs['llm_config'] = llm_config
    if runtime_flags:
        solver_kwargs['runtime_flags'] = runtime_flags
