---
## **Project ARC-Angel: Anti-Drift Ethos**

**Mission Statement:** To create a top-tier, completely offline ARC solver by leveraging online LLM-powered analysis to build a robust, self-contained reasoning engine.

### ## The Three Pillars

Every task we undertake must serve one of these three core principles. If it doesn't, it's project drift.

1.  **Offline is Law. 🚫🌐**
    The final Kaggle submission *must* be 100% self-contained and run without internet access. All online work, including LLM calls, is exclusively for pre-generating assets that the offline solver will use. We will never build a feature that relies on an API call in the final notebook.

2.  **Generate Online, Reason Offline. 🧠➡️🤖**
    Our strategy has two distinct phases, and we must respect the boundary. The **Online Phase** is for generating the `master_solution_log.jsonl`. Its purpose is to create a massive database of high-quality solutions and reasoning. The **Offline Phase** is for building the solver that uses those assets. We are currently focused on building the tools for the offline solver.

3.  **Reasoning over Recipes (RFT Focus). 🧑‍🍳 > 📖**
    Our ultimate goal is to solve **novel problems**. The Program Bank (the "recipe book") is important, but the **Generalized Reasoning Engine** (the "chef's intuition") is what will win the competition. Every heuristic we build must be guided by the principles of Relational Frame Theory (RFT), focusing on abstract, transferable rules (causality, comparison, spatial relations) rather than brittle, task-specific hacks.

---
## Definition of "Done" ✅

We are finished when we can check off all of the following:

* [ ] Master Solution Log: The online generation script has been run on all training tasks.
* [ ] DSL Interpreter: The `arc_dsl_lib.py` can successfully execute **all verified programs** from the solution log.
* [ ] Program Bank: A verified `program_bank.json` has been created and is accessible to the offline solver via a similarity search.
* [ ] Reasoning Engine: The top 5-10 most common, RFT-inspired reasoning patterns have been coded into a "Hypothesize and Test" engine.
* [ ] Final Submission: The complete, offline notebook has been packaged with all necessary assets, tested with the internet disabled, and successfully generates a valid `submission.json` file.
...
This repository is actively developed by **POPEYE THE SAILOR-DEV**, a jaunty, hyper-competent full-stack engineer. For more details on the development persona and workflow, see `GEMINI.md`.
