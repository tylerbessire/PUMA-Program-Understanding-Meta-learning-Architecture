...
class ARCSolver:
    ...
    def solve_task(self, task: Dict[str, List[Dict[str, List[List[int]]]]]) -> Dict[str, List[List[List[int]]]]:
        ...
        # --- Object Evolution Tracking (Super-Mode Enhancement) ---
        all_evolutions = []
        if self.enable_logging:
            self.logger.info("--- Running Object Evolution Tracking ---")
            tracker = ObjectTracker()
            feature_extractor = DetectorOrchestrator()
            for i, (in_grid, out_grid) in enumerate(train_pairs):
                input_feats = feature_extractor._extract_features(in_grid)
                output_feats = feature_extractor._extract_features(out_grid)
                evolutions = tracker.track_evolutions(
                    in_grid,
                    out_grid,
                ...
                self.logger.info(f"  Training Pair {i}: Found {len(evolutions)} object evolutions.")
                for ev in evolutions:
                    self.logger.info(f"    - Cost: {ev.cost:.2f}, Delta: {ev.delta}")
            self.logger.info("--- End Object Evolution Tracking ---")
        
        # --- End of Enhancement ---

        # Generate a hypothesis from the evolutions
        best_hypothesis = self._analyze_evolutions(all_evolutions)

        # If a strong hypothesis is found, use it to predict the test outputs
        if best_hypothesis:
            if self.enable_logging:
                self.logger.info("Applying induced hypothesis to test inputs.")
            attempt1: List[List[List[int]]] = []
            for test_input in test_inputs:
                predicted_grid = self._apply_hypothesis(test_input, best_hypothesis, training_stats)
                attempt1.append(to_list(predicted_grid))
            
            # For now, attempt 2 is the same as attempt 1.
            # A more advanced implementation would generate a second, diverse hypothesis.
            attempt2 = [row[:] for row in attempt1]
            ...
            return self._finalize_result(
                ...
                attempt1,
                attempt2,
                solved_training=True, # Assume solved if we have a hypothesis
            )

        # Fallback to existing logic if no hypothesis could be generated
        if self.enable_logging:
            self.logger.info("No strong hypothesis induced. Falling back to original solver logic.")
        ...
        # Fast RFT detector pass before expensive search
            ...
            if fast_signal and fast_signal.confidence > 0.85:
                if self._validate_rft_signal(fast_signal, train_pairs):
                    ...
                    if attempt1 and attempt2:
                        self.stats['transduction_successes'] = self.stats.get('transduction_successes', 0) + 1
                        if self.enable_logging:
                            self.logger.info(
                                "RFT detector '%s' solved task!",
                                fast_signal.detector_name,
                            )
                        return self._finalize_result(
                            task,
                            task_id,
            ...
            if best_hypothesis.confidence == 1.0:
                ...
                for test_input in test_inputs:
                    ...
                    if transformed is None:
                        break
                    output_list = to_list(transformed)
        ...
        if self.enable_logging:
            self.logger.info(
                "Task %s transduction=%s induction=%s",
                task_id,
                bool(self._detector_hint),
                solved_training,
            )
        if solved_training:
            self.stats['induction_successes'] = self.stats.get('induction_successes', 0) + 1
        ...
        if self.use_enhancements:
            try:
                if self.enable_logging:
                    ...
                    if getattr(self, "_detector_hint", None):
                        self.logger.info(
                            "Detector hint propagated to TTT: %s",
                            self._detector_hint,
                        )
                progs = synthesize_with_enhancements(
                    ...
                    detector_hint=getattr(self, "_detector_hint", None),
                )
                if getattr(self, "_detector_hint", None):
                    self.stats['detector_hints_used'] = self.stats.get('detector_hints_used', 0) + 1
    ...
    def _apply_rft_rule(
        ...
        if rule_type == "remove_colors_and_crop":
            ...
            pruned = grid.copy()
            if removable:
                mask = np.isin(pruned, removable)
                pruned[mask] = 0

            top = hypothesis.get("crop_top")
            left = hypothesis.get("crop_left")
            height = hypothesis.get("height")
            width = hypothesis.get("width")

            if None not in (top, left, height, width):
                top = int(top)
                left = int(left)
                height = int(height)
                width = int(width)
                ...
                if top < 0 or left < 0 or top + height > h or left + width > w:
                    return None
                return pruned[top : top + height, left : left + width].copy()
            ...
            return pruned[min_r : max_r + 1, min_c : max_c + 1].copy()

        if rule_type == "recolor_crop_subgrid":
            top = hypothesis.get("top")
            left = hypothesis.get("left")
            height = hypothesis.get("height")
            width = hypothesis.get("width")
            ...
            cropped = grid[top : top + height, left : left + width].copy()
            for src, dst in color_map.items():
                cropped[cropped == int(src)] = int(dst)
        ...
        if rule_type == "rotate_crop_subgrid":
            height = hypothesis.get("height")
            width = hypothesis.get("width")
            rotation_candidates = hypothesis.get("rotation_candidates")
            legacy_rotation = hypothesis.get("rotation_degrees")
            ...
            if rotation_candidates is None:
                rotation_candidates = [int(legacy_rotation) if legacy_rotation is not None else 0]

            if None in (height, width) or not rotation_candidates:
                return None
            ...
            h, w = grid.shape
            if height <= 0 or width <= 0 or height > h or width > w:
                return None

            rotation_candidates = [int(deg) for deg in rotation_candidates]
    ...
    def _apply_hypothesis(self, grid: Array, hypothesis: Hypothesis, training_stats: Dict[str, Any]) -> Array:
        """Applies a portfolio of rules from a hypothesis to transform a grid."""
        if not hypothesis or not hypothesis.program_sketch:
            return grid

        # Start with a copy of the input grid
        new_grid = grid.copy()
        background_color = training_stats.get("background_color", 0)

        # The order of rule application can be critical. This is a default logical order.
        rule_order = ['delete', 'resize', 'remap_colors', 'translate']

        for rule_type in rule_order:
            for rule in hypothesis.program_sketch:
                if rule['type'] != rule_type:
                    continue

                # We need to re-fetch objects for each rule type to ensure subsequent rules
                # act on the output of previous rules.
                objects = connected_components(new_grid)
                grid_before_rule = new_grid.copy()

                conditions = rule.get('condition', {})

                for obj in objects:
                    # Check if the object meets all conditions
                    if not self._object_matches_conditions(obj, conditions):
                        continue
                    
                    if self.enable_logging:
                        self.logger.info(f"  [Applying Rule] {rule['type']} on object with color {obj.get('color')}")

                    # Apply the rule to the matched object
                    if rule['type'] == 'delete':
                        for r, c in obj['pixels']:
                            new_grid[r, c] = background_color

                    elif rule['type'] == 'remap_colors':
                        to_c = rule['color_map'][obj['color']]
                        for r, c in obj['pixels']:
                            # Check color at the source to prevent re-coloring already changed pixels
                            if grid_before_rule[r, c] == obj['color']:
                                new_grid[r, c] = to_c
                        ...
                        for r, c in obj['pixels']:
                            new_r, new_c = r + vector[0], c + vector[1]
                            if 0 <= new_r < new_grid.shape[0] and 0 <= new_c < new_grid.shape[1]:
                                new_grid[new_r, new_c] = obj['color']

        return new_grid

    def _object_matches_conditions(self, obj: Dict[str, Any], conditions: Dict[str, Any]) -> bool:
        """Checks if a single object meets a set of rule conditions."""
        for key, value in conditions.items():
            if key == 'color' and obj.get('color') != value:
                return False
            if key == 'size' and len(obj.get('pixels', [])) != value:
                return False
            # Can be extended with shape_hash, etc.
        return True

    def _analyze_evolutions(self, all_evolutions: List[List["ObjectEvolution"]]) -> Optional[Hypothesis]:
        """Analyzes aggregated evolutions to induce a portfolio of transformation rules."""
        if not all_evolutions or not all_evolutions[0]:
            return None

        flat_evolutions = [ev for evolutions in all_evolutions for ev in evolutions]
        if not flat_evolutions:
            return None

        program_sketch = []
        from collections import Counter

        # --- Portfolio Rule Induction ---
        # Group evolutions by input object properties to find conditional rules.
        evolutions_by_color: Dict[int, List[ObjectEvolution]] = {}
        for ev in flat_evolutions:
            color = ev.input_obj.color
            if color not in evolutions_by_color:
                evolutions_by_color[color] = []
            evolutions_by_color[color].append(ev)

        # Find all plausible rules for each group
        for color, group_evolutions in evolutions_by_color.items():
            group_size = len(group_evolutions)

            # 1. Deletion Rule
            deletions = [ev for ev in group_evolutions if ev.output_obj is None]
            if len(deletions) / group_size > 0.6:
                rule = {'type': 'delete', 'condition': {'color': color}}
                program_sketch.append(rule)
                if self.enable_logging:
                    self.logger.info(f"  [Rule Induced] For color {color}, DELETE object")

            # 2. Color Change Rule
            color_deltas = [ev.delta.get('color') for ev in group_evolutions if 'color' in ev.delta]
            if color_deltas:
                (from_c, to_c), count = Counter(color_deltas).most_common(1)[0]
                if count / len(color_deltas) > 0.6:
                    rule = {
                        'type': 'remap_colors',
                        'condition': {'color': from_c},
                        'color_map': {from_c: to_c}
                    }
                    program_sketch.append(rule)
                    if self.enable_logging:
                        self.logger.info(f"  [Rule Induced] For color {from_c}, change to {to_c}")

            # 3. Translation Rule
            pos_deltas = [tuple(np.round(ev.delta.get('position')).astype(int)) for ev in group_evolutions if 'position' in ev.delta]
            if pos_deltas:
                most_common_translation, count = Counter(pos_deltas).most_common(1)[0]
                if count / len(pos_deltas) > 0.6:
                    rule = {
                        'type': 'translate',
                        'condition': {'color': color},
                        'vector': most_common_translation
                    }
                    program_sketch.append(rule)
                    if self.enable_logging:
                        self.logger.info(f"  [Rule Induced] For color {color}, translate by {most_common_translation}")

        if not program_sketch:
            if self.enable_logging:
                self.logger.info("  [Analysis] No high-confidence conditional rules were induced.")
            return None

        hypothesis = Hypothesis(
            transformation_type='object_evolution_portfolio',
            program_sketch=program_sketch,
            confidence=0.8  # Base confidence for a portfolio hypothesis
        )

        if self.enable_logging:
            self.logger.info(f"--- Evolution Analysis Complete: Generated Portfolio Hypothesis ---")
            self.logger.info(f"  - Rules: {hypothesis.program_sketch}")
            self.logger.info("-------------------------------------------------------------")

        return hypothesis
        if not all_evolutions:
            return {}

        # Aggregate all deltas
        all_deltas = [ev.delta for evolutions in all_evolutions for ev in evolutions]

        # --- Pattern Analysis ---
        # 1. Common Deltas: Find transformations that apply to all objects.
        #    (e.g., every object changes color from red to blue)
        # 2. Grouped Deltas: Group objects by properties (e.g., color, size) and
        #    find rules specific to those groups.
        #    (e.g., only red objects move)
        
        # For now, we'll do a simple frequency count of delta types.
        # This will be expanded into a full rule induction system.
        ...
        summary = {
            ...
            'position_changes': [],
            'shape_changes': 0,
            'total_evolutions': len(all_deltas)
        }

        for delta in all_deltas:
            if 'color' in delta:
                summary['color_changes'].append(delta['color'])
            if 'size' in delta:
                summary['size_changes'].append(delta['size'])
            if 'position' in delta:
                summary['position_changes'].append(delta['position'])
            if 'shape' in delta:
                summary['shape_changes'] += 1
        
        # Log the findings for now
        if self.enable_logging:
            self.logger.info("--- Evolution Analysis Summary ---")
            self.logger.info(f"  - Total Evolutions Tracked: {summary['total_evolutions']}")
            self.logger.info(f"  - Color Changes: {len(summary['color_changes'])}")
            self.logger.info(f"  - Size Changes: {len(summary['size_changes'])}")
            self.logger.info(f"  - Position Changes: {len(summary['position_changes'])}")
            self.logger.info(f"  - Shape Changes: {summary['shape_changes']}")
            self.logger.info("------------------------------------")
...
# [S:OBS v1] logging=structured fallback_metric=fallback_used pass
    ...
    def _finalize_result(
        ...
        result = {
            ...
            "attempt_2": attempt2,
            "metadata": {
                "solution_mode": self._last_solution_mode,
                "detector_hint": getattr(self, "_detector_hint", None),
            },
        }
