...
class HumanGradeReasoner:
    ...
    def __init__(self):
        ...
        # Cached descriptors of training outputs for better extraction matching
        self.output_histograms: List[np.ndarray] = []
    ...
    def analyze_task(self, train_pairs: List[Tuple[Array, Array]]) -> List[SpatialHypothesis]:
        ...
        self.hypotheses = []
        self._cache_output_signatures(train_pairs)
    ...
    def _cache_output_signatures(self, train_pairs: List[Tuple[Array, Array]]) -> None:
        """Pre-compute normalized color histograms of training outputs."""
        ...
        for _, out in train_pairs:
            flat = out.flatten()
            # Colours are 0-9 in ARC; ensure consistent length histograms
            hist = np.bincount(flat, minlength=10).astype(np.float32)
            total = np.sum(hist)
            if total > 0:
                hist /= total
            self.output_histograms.append(hist)
    ...
    def _generate_replacement_hypotheses(self, train_pairs: List[Tuple[Array, Array]], placeholders: List[Dict[str, Any]]):
        ...
        for placeholder in placeholders:
            ...
            if target_shape in target_shapes:
                ...
                # Cache reference outputs for later matching
                placeholder['reference_outputs'] = [
                    out.copy() for _, out in train_pairs if out.shape == target_shape
                ]
    ...
    def _find_best_extraction_region(self, inp: Array, target_shape: Tuple[int, int]) -> Array:
        ...
        for r in range(h - target_h + 1):
            for c in range(w - target_w + 1):
                ...
                # Boost score when region matches training output color distribution
                if self.output_histograms:
                    region_hist = np.bincount(region.flatten(), minlength=10).astype(np.float32)
                    region_hist /= max(1.0, region.size)
                    # Compare against closest training histogram (cosine similarity proxy)
                    best_similarity = 0.0
                    for hist in self.output_histograms:
                        # Use dot product of normalized histograms as similarity (range 0..1)
                        similarity = float(np.dot(region_hist, hist))
                        if similarity > best_similarity:
                            best_similarity = similarity
                    # Blend similarity with existing score to bias towards known patterns
    ...
    def _find_reference_match(
        ...
        """Find an exact match of any reference output within the input grid."""
        ...
        for ref in reference_outputs:
            ...
            if ref_arr.shape != target_shape:
                continue
            for r in range(h - target_h + 1):
                for c in range(w - target_w + 1):
                    region = inp[r:r+target_h, c:c+target_w]
                    if np.array_equal(region, ref_arr):
                        return region.copy()
        return None
    ...
    def _generate_relational_hypotheses(self, train_pairs: List[Tuple[Array, Array]]):
        ...
        for fact in relational_sources:
            ...
            hop = max(abs(translation[0]), abs(translation[1]))
            if hop > 4:
                continue
